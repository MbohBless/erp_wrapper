...
import { type PurchaseInput, createPurchase } from "@/lib/purchases";
...
function NewBillForm() {
  ...
  function pickItem(i: number, sku: string) {
    ...
    setRow(i, { item_code: sku, rate: p?.purchase_price != null ? String(p.purchase_price) : "" });
  }
    ...
    mutationFn: () => {
      const input: PurchaseInput = {
        ...
        items: rows
          ...
          .map((r) => ({ item_code: r.item_code.trim(), qty: parseFloat(r.qty) || 0, rate: parseFloat(r.rate) || 0 })),
      };
      return createPurchase(token as string, input);
    },
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <nav className="flex items-center gap-2 text-xs muted">
              <span>{t("purchases.title")}</span><span>›</span><span className="text-ink">{t("purchases.new.title")}</span>
            </nav>
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-heading font-semibold">{t("purchases.new.title")}</h1>
        ...
        {tab === "details" && (
          <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-5">
            <div>
              ...
              <input className={`${FIELD} opacity-70`} value="ACC-PINV-.YYYY.-" disabled />
            </div>
            <div>
